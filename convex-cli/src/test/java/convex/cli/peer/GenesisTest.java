...
public class GenesisTest {
	...
	@Test public void testGenesisPeer() throws TimeoutException, InterruptedException, IOException, LaunchException, ConfigException {
		...
		CLTester tester =  CLTester.run(
				...
				"--pg-url", PostgresTestHelper.jdbcUrl(),
				"--pg-user", PostgresTestHelper.username(),
				"--pg-password", PostgresTestHelper.password(),
				"--pg-reset",
				"--key", expectedKey,
				"--keypass", new String(KEY_PASSWORD),
				"--keystore", KEYSTORE_FILENAME
		...
		try (PostgresStore store = PostgresTestHelper.createStore(false)) {
